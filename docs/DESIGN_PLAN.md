...
- **牌桌布局（麻将桌式）**：以玩家为视角，**上家**在画面**左侧**，**对家**在**上方**，**下家**在画面**右侧**，玩家自己在下方；即四家呈「左—上—右—下」环绕牌桌，与麻将桌一致。**左右两家**（上家、下家）的手牌与副露为**竖排**（VBoxContainer），且**牌面旋转 90°**（上家 -90°、下家 90°），对家与玩家为横排不旋转。**四家手牌区域**均位于各自边（上/下/左/右）的**中间**，左右两侧用上下弹性留白、上下两侧用水平居中对齐实现。
- **信息区布局**：对局画面中央/左侧的文本信息（牌库数量、当前回合等）须分行、分块显示（如使用 VBoxContainer），避免多个 Label 叠在同一位置造成重叠。
