...
- **牌组件**：单张牌组件需支持「牌面」与「牌背」两种显示模式（如 `face_down`），牌背用于他家手牌，牌面用于己方手牌、副露及当前打出的牌。
...
- **响应阶段与 response_order**  
  有人出牌后进入响应阶段，**仅将「出牌者以外的三家」**按逆时针顺序放入 `response_order`（即循环 `PLAYER_COUNT - 1` 次），从出牌者下家起依次询问胡 / 杠 / 碰 / 吃或过。出牌者本人不再进入响应队列，否则会出现「三家都过后又轮到出牌者响应」的错误。
- **响应阶段与 state_changed**  
...
- **GameState**（或 `game_state.gd`）：当前对局状态单例。
  ...
  - 方法：发牌、出牌（同时写入打出历史）、摸牌、响应（吃碰杠胡）、结算、重置一局。
...
- **Table.gd**：
  ...
  - 玩家操作：点击手牌出牌、或选择「吃/碰/杠/胡」时，调用 GameState 的对应接口；若合法则更新状态并推进回合（或切换到响应者），否则提示不合法。
